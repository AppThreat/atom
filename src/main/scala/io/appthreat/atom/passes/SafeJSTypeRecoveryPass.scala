...
class SafeJSTypeRecoveryPass(cpg: Cpg, config: XTypeRecoveryConfig = XTypeRecoveryConfig(enabledDummyTypes = false))
...
private class SafeJSTypeRecovery(cpg: Cpg, state: XTypeRecoveryState) extends XTypeRecovery[File](cpg, state) {
...
private class RecoverForJavaScriptFile(cpg: Cpg, cu: File, builder: DiffGraphBuilder, state: XTypeRecoveryState)
  ...
  override protected def prepopulateSymbolTableEntry(x: AstNode): Unit = x match {
    case x @ (_: Identifier | _: Local | _: MethodParameterIn)
      ...
      val resolvedTypeHints: Set[String] =
        ...
        else if (cpgTypeFullName.nonEmpty) {
          if (!cpgTypeFullName.head.startsWith("{")) symbolTable.put(x, cpgTypeFullName)
          else null
        } else symbolTable.put(x, x.getKnownTypes)
      if (resolvedTypeHints != null && resolvedTypeHints.sizeIs == 1 && !resolvedTypeHints.contains(typeFullName)) {
        if (!resolvedTypeHints.head.contains(".js")) {
  ...
  override protected def prepopulateSymbolTable(): Unit = {
    ...
    (cu.ast.isParameter.whereNot(_.nameExact("this")) ++ cu.ast.isMethod.methodReturn).filter(hasTypes).foreach { p =>
      ...
      p match {
        ...
        case _: MethodReturn if resolvedHints.sizeIs == 1 && !resolvedHints.head.contains(".js") =>
  ...
  override protected def associateInterproceduralTypes(
    ...
    if (fieldFullName.startsWith("req:") || fieldFullName.startsWith("request:")) {
      symbolTable.append(i, Set("Request", "http.IncomingMessage"))
    } else if (fieldFullName.startsWith("res:") || fieldFullName.startsWith("response:")) {
